/*
 * Created by Ion Drive Software Ltd from sources at
 * http://github.com/LazarSoft/jsqrcode
 */

/*
 * Copyright 2014 Ion Drive Software Ltd
 *
 * Copyright 2011 Lazar Laszlo
 *
...
(function (window) {
  ...
  function ErrorCorrectionLevel(ordinal, bits, name) {
    this.ordinalRenamedField = ordinal;
    this.bits = bits;
    this.name = name;
    ...
    this.ordinal = function() {
      return this.ordinalRenamedField;
    };
  }
  ...
  qrcode.decodeUrl = function(s) {
    var escaped = '';
    try {
      escaped = encodeURIComponent(s);
    } catch (e) {
      escaped = s;
    }
    var ret = '';
  ...
  qrcode.decodeUtf8 = function(s) {
    if (qrcode.isUrl(s)) {
      return qrcode.decodeUrl(s);
    }
    return s;
  };
  ...
  qrcode.process = function(ctx) {
    ...
    /*for (var y = 0; y < qRCodeMatrix.bits.Height; y++)
    {
      for (var x = 0; x < qRCodeMatrix.bits.Width; x++)
      {
        ...
        qrcode.imagedata.data[point] = qRCodeMatrix.bits.getRenamed(x,y)?0:0;
        qrcode.imagedata.data[point+1] = qRCodeMatrix.bits.getRenamed(x,y)?0:0;
        qrcode.imagedata.data[point+2] = qRCodeMatrix.bits.getRenamed(x,y)?255:0;
      }
    }*/
    if (qrcode.debug) {
      ctx.putImageData(qrcode.imagedata, 0, 0);
  ...
  canvas.println('Guessed mode: ' + mode); */
        ...
        switch (mode) {
          ...
          case MODE_8BIT_BYTE:
            ...
            //sbyte[] tempsbyteArray3;
            var tempsbyteArray3 = this.get8bitByteArray(dataLength);
            output.push(tempsbyteArray3);
            //output.Write(SystemUtils.ToByteArray(tempsbyteArray3), 0, tempsbyteArray3.Length);
            break;
